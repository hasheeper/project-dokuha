/**
 * DOKUHA SillyTavern status iframe host.
 */
(function () {
  'use strict';
  ...
  function appendQueryParams(url, params = {}) {
    ...
    try {
      const parsed = new URL(url, /^https?:\/\//i.test(url) ? undefined : 'https://dokuha.local');
  ...
  RUNTIME.createStatusHost = function createStatusHost(stateService: any, config: any = {}) {
    ...
    const injectStatusHost = !isDisabled(config.injectFixedStatus)
      ...
      && !isEnabled(ROOT.DOKUHA_DISABLE_STATUS_HOST)
      && !isEnabled(UI_ROOT.DOKUHA_DISABLE_STATUS_HOST);
    ...
    function handleMessage(event) {
      ...
      if (isPatch) {
        ...
        if (!target) return;
        void patchStateFromMessage(target, data);
        return;
      }
